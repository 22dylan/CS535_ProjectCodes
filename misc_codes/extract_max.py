...
path_to_data = os.path.join(os.getcwd(), '..', 'data', 'CHS_Storms')
path_out = os.path.join(os.getcwd(), '..', 'data', 'Max_Surge_nyc', 'CSVs')
...
for i, storm in enumerate(missing_storms):
	...
	df_out = df_out[df_out['SP_Longitude'].between(xmin, xmax, inclusive=True)]
	df_out = df_out[df_out['SP_Latitude'].between(ymin, ymax, inclusive=True)]
